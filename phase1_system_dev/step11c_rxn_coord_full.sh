...
python3 - "$prmtop" "$traj" "$outdir" "$traj2" <<'PY'
...
def frame_from(path,hdr,cv,i):
    """One frame of one trajectory. Used for both files so there is a
    single definition and no parallel version that can drift from it."""
    off=cv["begin"]+i*hdr["recsize"]
    return np.array(np.memmap(path,dtype=">f4",mode="r",offset=off,shape=(natom,3))[:maxidx],float)
def frame(i):
    return frame_from(traj,H,cvar,i)
...
def frame_any(i):
    if i < N1:
        return frame_from(traj,H,cvar,i)
    return frame_from(traj2,H2,cvar2,i-N1)
print("  trajectory 1: %d frames, trajectory 2: %d, total %d"%(N1,N2,NT))
os.makedirs(outdir,exist_ok=True)
per={rid:{k:np.empty(NT) for k in list(PAIRS)+["rc"]} for rid,_ in cha}
for i in range(NT):
    x=frame_any(i)
    for rid,amap in cha:
        d={}
...
with open(os.path.join(outdir,"rxn_coord_per_frame_full.dat"),"w") as f:
...
print("  CHA copies %d   frames %d"%(len(cha),NT))
...
print("  wrote", os.path.join(outdir,"rxn_coord_per_frame_full.dat"))
PY
echo "STEP 11c DONE"
