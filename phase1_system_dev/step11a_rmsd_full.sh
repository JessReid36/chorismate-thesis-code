...
outdat="$outdir/rmsd_vs_time_full.dat"

for f in "$prmtop" "$traj" "$traj2"; do [[ -s "$f" ]] || { echo "FAIL missing $f"; exit 1; }; done

python3 - "$prmtop" "$traj" "$outdat" "$traj2" <<'PY'
...
def frame_from(path,hdr,cv,lv,i):
    """One frame of one trajectory. Used for both files so there is a single
    definition and no parallel version that can drift from it."""
    off=cv["begin"]+i*hdr["recsize"]
    xyz=np.array(np.memmap(path,dtype=">f4",mode="r",offset=off,shape=(natom,3))[:n_solute],float)
    box=None
    if lv is not None:
        bo=lv["begin"]+i*hdr["recsize"]; box=np.array(np.memmap(path,dtype=">f8",mode="r",offset=bo,shape=(3,)),float)
    return xyz,box
def frame(i):
...
# The two files are one trajectory: the extension restarted from the first
# run's restart with velocities. Production frame i is read from the first file
# if i is below its record count and from the second at i-N1 otherwise. The
# reference is frame 0 of the FIRST file in both cases, so the whole trace is
# measured against one structure and the values are comparable throughout.
H2=nc_open(traj2); assert H2["atom"]==natom, "atom mismatch in the extension"
...
print("  trajectory 1: %d frames, trajectory 2: %d frames, total %d"%(N1,N2,NT))
print("  reference: frame 0 of the first trajectory, for the whole trace")
x0,b0=frame(0); ref=image(x0,b0)[bb_idx]; rms=np.empty(NT)
for i in range(NT):
    xi,bi=frame_any(i); rms[i]=kabsch(image(xi,bi)[bb_idx],ref)
...
w=max(1,NT//10); first=float(rms[:w].mean()); last=float(rms[-w:].mean())
print("STEP 11a - protein backbone RMSD vs production frame 0")
print("  backbone atoms %d   protein molecules %s   frames %d"%(len(bb_idx),prot_mols,NT))
