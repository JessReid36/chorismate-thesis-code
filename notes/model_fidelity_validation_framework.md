...
The evidence here is the located transition state itself. The pipeline yields a converged first-order saddle point with a single dominant imaginary vibrational mode (−313 cm⁻¹) corresponding to the concerted C–O bond cleavage and C–C bond formation of the Claisen rearrangement, connecting a clean chorismate reactant to an exothermic prephenate product; a full numerical frequency calculation over the reduced active region (612 displacements per endpoint) confirms both the reactant and the product as genuine minima with no imaginary frequencies, so all three stationary points are frequency-characterised. The potential-energy barrier for the studied snapshot, computed on a consistent reduced region in which reactant, transition state, and product share the same movable atoms and embedding, is 16.00 kcal/mol.

Two points of honesty about this number. First, it is a *potential-energy* barrier for a single conformational snapshot, not a free energy and not an ensemble average; the appropriate experimental comparator is the activation enthalpy ΔH‡ = 12.7 kcal/mol rather than the activation free energy ΔG‡ = 15.4 kcal/mol, since a single static path does not contain the entropic and conformational-averaging contributions that separate the two. Against ΔH‡ the value is approximately 3.3 kcal/mol high — within the range reported in the prior QM/MM literature for this enzyme at comparable levels of theory, and on the high side of it, consistent with a single high-barrier conformer. Second, the computed 16.00 sits just above the experimental ΔG‡ of 15.4; these are different physical quantities, so their proximity is not evidence of accuracy and should not be presented as agreement.
...
Distinct from whether the model matches the enzyme is whether the reported number is a genuine property of the model or an artefact of one algorithm. This is established by locating the same transition state through independent methods. Here the restrained adiabatic scan (barrier maximum ≈ 16.37 kcal/mol), the nudged-elastic-band climbing image (≈ 15.94 kcal/mol), and the reduced-active-region saddle optimisation referenced to the consistent reduced-region reactant (16.00 kcal/mol) agree to within about half a kcal/mol. The imaginary frequency of the converged saddle (−313 cm⁻¹) is close to the value obtained in independent prior work on the same system.
...
- A verified transition state: converged saddle, single reaction-coordinate imaginary mode, connecting clean reactant and exothermic product, with both endpoints confirmed as frequency-verified minima on the same reduced region.
