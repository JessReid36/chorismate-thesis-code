...
FROZEN = list(range(24,38))  # surrogates fixed at certified positions

if not RUN:
    print("\n=== DRY RUN OK (MM molecular surrogate) — %d LJ pairs, QM -2, %d frozen MM. ===" % (npair,len(FROZEN)))
    sys.exit(0)

Optimizer(theory=qmmm, fragment=frag, coordsystem="hdlc",
          frozenatoms=FROZEN, maxiter=200, charge=-2, mult=1)
frag.write_xyz("k2_mmsurr_relaxed.xyz")
