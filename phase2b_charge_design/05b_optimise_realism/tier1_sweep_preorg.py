...
def load_apot(path):
    ...
    with open(path) as fh:
        ...
        for ln in fh:
            p=ln.rstrip("\n").split("\t")
            a[int(p[0])]=float(p[2])   # a_pot (unnormalised, physical)
    return a
...
if __name__=="__main__":
    ...
    a=ap.parse_args()
    grid=load_dv_grid(a.dv_grid); apot=load_apot(a.apot_tsv); o3=ether_o(a.reactant_xyz)
    ...
    print("preorg penalty sweep: distorting drive D = sum q*a_pot ; a_pot,i = V_i(C1)-V_i(C6)")
