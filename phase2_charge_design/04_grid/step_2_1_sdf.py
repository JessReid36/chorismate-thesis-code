#!/usr/bin/env python3
# Phase 2 step 2.1 (Option 3, union envelope) - union-of-spheres SDF over the reactant, TS AND product
# geometries so every point outside the envelope clears the substrate vdW surface at EVERY point on the
# reaction path (the designed external charges are static while the substrate reacts). SDF(r) = min over
# ALL atoms of ALL geometries (|r - r_atom| - R_vdw). Downstream shells/thinning unchanged.
import sys
import numpy as np
...
def read_union(paths):
    all_els, all_xyz, per_geom = [], [], []
    for p in paths:
        els, xyz = read_xyz(p)
        all_els += els; all_xyz.append(xyz); per_geom.append((p, len(els)))
    return all_els, np.vstack(all_xyz), per_geom

def build_sdf(els, xyz, voxel=0.30, margin=5.0):
    radii = np.array([BONDI[e] for e in els])
    rmax = radii.max()
    lo = xyz.min(0) - (rmax + margin); hi = xyz.max(0) + (rmax + margin)
...
def verify(sdf, grid, els, xyz, radii):
    lo, voxel, dims = grid; checks = {}; centre_vals = []
    for (ax,ay,az) in xyz:
    ...
    ix = int(np.argmax(xyz[:,0])); r = radii[ix]; probe = xyz[ix] + np.array([r, 0, 0])
...
if __name__ == "__main__":
    args = [a for a in sys.argv[1:] if not a.replace('.','',1).isdigit()]
    nums = [a for a in sys.argv[1:] if a.replace('.','',1).isdigit()]
    paths = args if args else ["reactant.xyz", "ts.xyz", "product.xyz"]
    voxel = float(nums[0]) if nums else 0.30
    els, xyz, per_geom = read_union(paths)
    print(f"union geometries: {[p for p,_ in per_geom]}")
    print(f"pooled atoms: {len(els)} ({' + '.join(str(n) for _,n in per_geom)})  elements: {sorted(set(els))}")
    sdf, grid, radii = build_sdf(els, xyz, voxel=voxel); lo, vox, dims = grid
